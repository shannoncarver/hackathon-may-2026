...
## Status
One canonical skill landed: `routing`. Future skills follow the same folder shape. The `auth0-management` skill (merged from `auth0-logs`, `auth0-stats`, `auth0-sec` per Decision 0025) — owned by `eng-ai (17)` with secondary review by `eng-security-iam (12)` for credential setup — adds frontline Auth0 sandbox query capability per [Decision 0014](../decisions/0014-auth0-logs-skill.md).
