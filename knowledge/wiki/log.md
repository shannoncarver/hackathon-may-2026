...
- PR: [shannoncarver/linq-platform-mcp#1](https://github.com/shannoncarver/linq-platform-mcp/pull/1) (merged squash → `493d5e3a` on `main`).
- Bootstrap stacks deployed via local AWS CLI (per session-locked decision D13 — short-lived session credentials inlined per-command, no `~/.aws/credentials` profiles):
  - `platform-bootstrap` @ `631916786699` — `CREATE_COMPLETE`. Output `GhaDeployRoleArn = arn:aws:iam::631916786699:role/gha-deploy-platform`.
  - `product-bootstrap` @ `529394632305` — `CREATE_COMPLETE`. Output `GhaDeployProductRoleArn = arn:aws:iam::529394632305:role/gha-deploy-product`.
- `deploy-main.yml` first-run after merge: <https://github.com/shannoncarver/linq-platform-mcp/actions/runs/25384461875> — `success`. Logs show GitHub Actions assumed `gha-deploy-platform` via OIDC (`Arn: arn:aws:sts::631916786699:assumed-role/gha-deploy-platform/gha-deploy-main-25384461875`), validated `infra/master.yaml`, and idempotent re-deploy reported "No changes to deploy. Stack platform-bootstrap is up to date" — round-trip clean.
- `pr.yml` enforces eight required-check job IDs matching [`implementation/11-repo-layout.md`](../../docs/research/0015-centralized-platform-mcp/implementation/11-repo-layout.md) §2.3 (`cfn-lint`, `cfn-nag`, `actionlint`, `unit-tests`, `contract-tests`, `schema-validation`, `sdk-typescript-build`, `sdk-python-build`). Real bodies for the three lint jobs; the five test/build jobs are echo-stub jobs in Phase B — the contract is the job NAME so the deferred D8 branch-protection rule lights up cleanly when Phase C deliverables land.
- Two CI-only failures fixed mid-PR: `rhysd/actionlint@v1` is not a published version tag (replaced with the canonical bash-downloader pattern from <https://github.com/rhysd/actionlint/blob/main/docs/usage.md>); the cfn-lint glob `infra/**/*.yaml` choked on the seven Phase A header-only nested-stack stubs (narrowed to `infra/master.yaml infra/stacks/bootstrap/*.yaml` until Phase C fills the bodies).
- Spec deviation discovered during deploy: AWS allows only one OIDC provider per URL per account, and both target accounts already had a GitHub Actions OIDC provider from prior integrations. Both bootstrap templates now carry a `CreateOidcProvider` parameter (default `false`) and reference the deterministic provider ARN `arn:aws:iam::<account>:oidc-provider/token.actions.githubusercontent.com` regardless of who created it. Documented in the linq-platform-mcp `CHANGELOG.md` Known gaps.
- Other Phase B deviations recorded in `CHANGELOG.md`: simplified IAM scope on the deploy roles (`*FullAccess` instead of the §2.4 production lockdown — tightening requires nested-stack role names from Phase C); deferral of D8 (branch protection) and D9 (signed-commit enforcement) to Phase C+; deferral of `04-registry-seed.yaml` body (D10); `deploy-main.yml` Phase B variant per D11 (validate-template + idempotent bootstrap re-deploy, not master deploy); `schema-validate` (§2.2) renamed to `schema-validation` (§2.3 wins).
- Coordinator: `10-eng-principal` (dispatch + AWS CLI deploy + GH workflow verification — specialist lacks shell access). Writer: `11-eng-cloudops` (six-file authoring per the 13-field schema at `schemas/agents/11-eng-cloudops.schema.json`).
