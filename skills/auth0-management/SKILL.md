...
description: Auth0 Management API queries on the LINQ sandbox tenant — logs, health stats, security inspection, app/client config, and per-user identity records. Use for: failed logins, auth0 logs, authentication failures, locked-out users, verify user can authenticate, auth0 health, login volume, MAU, monthly active users, failure rate, MFA adoption, top connections, blocked IPs, brute-force policy, breached-password policy, suspicious IP throttling, auth0 security posture, user record lookup, ERP V4 client config, application callbacks, or any auth-related incident triage, authentication activity audit, or security inspection on the LINQ sandbox tenant.
